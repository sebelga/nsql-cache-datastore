...
module.exports = ds => {
    // Since v2.0.0 of @google/datastore the isInt helper is accessed through googleDatastore object
    const isInt = ds ? ds.isInt || ds.googleDatastore.isInt : Function.prototype;

    const parseValue = val => (isInt(val) ? `DS_INT(${val.value})` : val);
    ...
    const datastoreAdapter = {
         ...
         */
        getEntity(keys) {
            return ds.get(keys).then(([entities]) => entities);
        },

        /**
         * Just before returning the entities (from cache or the fetch handler)
         * this method is called. As the Google Datastore library return the entities
         * in an Array at position 0, we return the same interface
         * @param {*} entities The entities array to be returend
         */
        postGetEntity(entities) {
        ...
        wrapClient(cache) {
             ...
             * To avoid to call twice the gsCache.read like this
             ...
             */
            datastoreAdapter.getEntityUnWrapped = (...args) => originalGet(...args).then(([entities]) => entities);

            ds.get = (keys, options = {}) => {
                if (!isCacheOn(options, cache.config)) {
                    return originalGet(keys);
                }
                return cache.keys.read(keys, options.cache, datastoreAdapter.getEntityUnWrapped);
            };
