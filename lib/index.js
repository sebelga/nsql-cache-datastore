...
module.exports = ds => {
    const datastoreAdapter = {
        /**
         * Get one or multiple (array) entities by Keu
         *
            ...
            ds.save = (entities, options = {}) =>
                originalSave(entities, options).then(res => {
                    ...
                    return Promise.all([
                        ...
                        isMultiple
                            ? cache.queries.clearQueriesEntityKind(entities.map(({ key }) => key.kind))
                            : cache.queries.clearQueriesEntityKind(entities.key.kind),
                    ]).then(() => res);
                });
            ...
            ds.delete = keys =>
                originalDelete(keys).then(res => {
                    ...
                    return Promise.all([
                        ...
                        isMultiple
                            ? cache.queries.clearQueriesEntityKind(keys.map(key => key.kind))
                            : cache.queries.clearQueriesEntityKind(keys.kind),
                    ]).then(() => res);
                });
        },
    };
